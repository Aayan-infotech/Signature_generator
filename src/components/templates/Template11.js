  ...
  return (
    <div style={{ fontFamily: data.font, color: data.color, fontSize: getFontSize(), lineHeight: getSpacing(), maxWidth: '800px', padding: '20px', borderRadius: '10px', backgroundColor: '#fff', boxShadow: '0px 4px 6px rgba(0, 0, 0, 0.1)' }}>
      ...
      <div style={{ textAlign: 'center', padding: '20px' }}>
        <h2 style={{ margin: 0, fontSize: '1.5rem', fontWeight: 'bold', color: '#2b5dad' }}>{data.name}</h2>
        ...
        <div style={{ marginTop: '20px', padding: '20px', backgroundColor: '#2b5dad', color: '#fff', borderRadius: '10px' }}>
          <p style={{ margin: '5px 0' }}><FaPhone />  {data.phone} {data.phone2}</p>
          <p style={{ margin: '5px 0' }}><FaGlobe /> {data.website} <FaEnvelope /> E {data.email}</p>
          <p style={{ margin: '5px 0' }}><FaMapMarkerAlt /> A {data.address}</p>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-evenly', marginTop: '20px', color: '#2b5dad' }}>
